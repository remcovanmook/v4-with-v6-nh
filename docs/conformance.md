...
| 4 | Lease expiry: host SHOULD remove 192.0.0.11 gateway and cease IPv6-based resolution | Conformant | v4gwd `--require-sentinel`: RTM_DELROUTE of the sentinel triggers withdrawal (`Interface.reconcile`; lab test T3). networkd: removed by its existing DHCPv4 route mark/sweep on lease loss/renewal. freebsd: `-f` state file from `dhclient-exit-hooks`, or `-r` FIB check |
